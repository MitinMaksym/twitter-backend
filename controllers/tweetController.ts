...
class TweetController {
  ...
  createTweet: RequestHandler = catchAsync(
    async (req: CustomRequestType, res) => {
      const user = req.user
  ...
  updateTweetById: RequestHandler = catchAsync(
    async (req: CustomRequestType, res, next) => {
      ...
      if (String(tweet?.user) !== req.user?.id) {
        return next(
          new AppError('User is allowed to update only own tweets', 403)
        )
      }
  ...
  deleteTweetById: RequestHandler = catchAsync(
    async (req: CustomRequestType, res, next) => {
      const tweet = await Tweet.findById(req.params.id)
      if (!tweet) {
      ...
      if (String(tweet?.user) !== req.user?.id) {
        return next(
          new AppError('User is allowed to delete only own tweets', 403)
        )
      }
